...
Enhancements:
    ...
    - More specifically, the use of a callback function that is a non-static member function of a class
        such that the callback function gets access to all of the class-instance's public and private members
    - Using a constraint pool to store constraints and add them later to the problem
    - Storing and loading MIP solutions
    - Enable/disable LP warm start
...
public:
    ...
    // Enhancement 1: Callback functions for integer solutions
    void intSolCallback(XpressProblem& p);
...
private:
    ...
    // Auxiliary decomposition variables in the main problem, lower bound on the second stage costs
    std::vector<Variable> theta_s;
    ...
    // Enhancement 2: Multi-cut L-shaped method instead of single-cut L-shaped method
    void addMultiOptimalityCutsToMainProb(std::vector<std::vector<double>>& optCutLhs_s, 
                                                        std::vector<double>& optCutRhs_s);
    void addOptimalityCutToMainProb(int s, std::vector<double>& optCutLhs, double& optCutRhs);
    void generateMultiOptimalityCuts(std::vector<std::vector<double>>& optCutLhs_s, 
                                     std::vector<double>& optCutRhs_s, bool calledFromCallback);

    // Enhancement 3: Save the MIP sol and load it in the next iteration for improved MIP heuristics
    std::vector<double> savedMipSol = {};

    // Enhancement 4: Constraint pool to store all optimality cuts generated during callbacks
    std::vector<double> constraintPoolRhsValues = {};
    std::vector<std::vector<double>> constraintPoolLhsMatrix = {};
    // Functions to add constraints to the pool, add the pool to the problem, and clear the pool
    void clearConstraintPool();
    void addConstraintPoolToMainProb();
    void addConstraintsToPool(const std::vector<std::vector<double>>& newConstraints, 
...
BRP_LShapedMethod::BRP_LShapedMethod(XpressProblem& mainProb,
        ...
        std::vector<double>& q2_i, std::vector<std::vector<double>>& d_s_i) 
     : mainProb(mainProb), c_i(c_i), b_i(b_i), p_s(p_s), c_ij(c_ij), 
...
void BRP_LShapedMethod::runLShapedMethod(bool verbose, bool printSolutions) {
    ...
    // add auxiliary variables theta to the main problem
    this->theta_s = mainProb.addVariables(NR_SCENARIOS).withLB(XPRS_MINUSINFINITY)
                                                         .withType(ColumnType::Continuous).toArray();
    // Add each theta to the objective, each with coefficient 1.0
    for (int s=0; s<NR_SCENARIOS; s++) {
        theta_s[s].chgObj(1.0);
    }

    /******************** Iteration 0: Solving all Sub Problems **********************/
    // Initialize objects to store the optimality cuts in
    std::vector<double> optCutRhs(NR_SCENARIOS, 0.0);
    std::vector<std::vector<double>> optCutLhsCoeffs(NR_SCENARIOS, std::vector<double>(NR_1ST_STAGE_VARIABLES, 0.0));

    // Generate the optimality cuts for the first time
    generateMultiOptimalityCuts(optCutLhsCoeffs, optCutRhs, false);
    ...
    while (true) {
        ...
        // Update main problem with last iteration's optimality cuts then -
        addMultiOptimalityCutsToMainProb(optCutLhsCoeffs, optCutRhs);
        solveMainProb(true);

        // After the main prob has been solved, we empty the constraint pool
        addConstraintPoolToMainProb();

        // Generate new optimality cuts
        generateMultiOptimalityCuts(optCutLhsCoeffs, optCutRhs, false);
...
void BRP_LShapedMethod::makeInitialMainProbFormulation() {
    ...
    /* CALLBACK */
    // The intSolCallback function actually takes 2 arguments instead of 1, as the first is the pointer to
    // the class instance itself `this`. Therefore, bind `this` as 1st argument, and the 2nd argument
    // `placeholder::_1` will be provided by the API when the callback is triggered
    auto bindedIntSolCallback = std::bind(&BRP_LShapedMethod::intSolCallback, this, std::placeholders::_1);
    mainProb.callbacks.addIntsolCallback(bindedIntSolCallback);
}
 ...
 *                        If `solveRelaxation` is false, the MIP problem is solved with a 1% optimality gap.
...
void BRP_LShapedMethod::solveMainProb(bool solveRelaxation) {
    ...
    if (solveRelaxation) {
        ...
        // Stop the MIP solver after 1% optimality gap (we do not care too much about good integer solutions)
        mainProb.controls.setMipRelStop(0.01);
        // Load the saved MIP solution from previous iteration. This is done to improve MIP-search heuristics
        if (savedMipSol.size() > 0) {
            mainProb.loadMipSol(savedMipSol);
        }
        // solve the MIP
        mainProb.optimize();
    ...
    if (mainProb.attributes.getOriginalCols() == x_i.size()) {
        ...
        this->lastThetaValue = BrpUtils::mySum(mainProb.getSolution(this->theta_s));
    }
...
/**
 * Adds multi-optimality cuts to the main problem. It loops over all generated cut-coefficients from
 * each scenario and adds them as constraints to the main problem.
 * 
 * @param optCutLhs_s The matrix `E` representing the left-hand side coefficients of the optimality cuts to
 *                    be added. One row per scenario, each row has a coefficient for each 1st-stage variable.
 * @param optCutRhs_s The vector `e` representing the right-hand side coefficients of the optimality cuts.
 *                    One row per scenario, each row has only one coefficient.
 */
void BRP_LShapedMethod::addMultiOptimalityCutsToMainProb(std::vector<std::vector<double>>& optCutLhs_s,
                                                           std::vector<double>& optCutRhs_s) {
    std::cout << "\tAdding optimality cuts" << std::endl;

    // Loop over all saved coefficients from each each scenario s, and add as a constraint to the main problem
    for (int s=0; s<NR_SCENARIOS; s++) {
        addOptimalityCutToMainProb(s, optCutLhs_s[s], optCutRhs_s[s]);
    }
}
 ...
 * 
 * @param s The index of the scenario corresponding to the cut, such that we know which theta variable to use.
 ...
 */
void BRP_LShapedMethod::addOptimalityCutToMainProb(int s, std::vector<double>& optCutLhs, double& optCutRhs) {
    ...
    mainProb.addConstraint(scalarProduct(x_i, optCutLhs) + theta_s[s] >= optCutRhs);

    if (printSolutions) {
        std::cout << "\t\t" << (scalarProduct(x_i, optCutLhs) + theta_s[s]).toString() << " >= " << optCutRhs << std::endl << std::endl;
    }
}

/**
 * This function solves subproblems and generates optimality cuts based on the dual values of the
 ...
 * coefficients for the optimality cuts can be computed.
 * 
 * @param optCutLhs_s A reference to the matrix representing the left-hand side coefficients of
 *                    all optimality cuts (one row for each scenario).
 * @param optCutRhs_s A reference to a vector representing the right-hand side values of the
 *                    optimality cuts (one coefficient for each scenario).
 * @param calledFromCallback A boolean indicating whether the function is called from a callback.
 *                           This is only used for printing purposes.
 * @return void, but the function returns/stores the optimality cuts in the provided references
 *         for optCutLhs_s and optCutRhs_s.
 */
void BRP_LShapedMethod::generateMultiOptimalityCuts(std::vector<std::vector<double>>& optCutLhs_s_i,
                                        std::vector<double>& optCutRhs_s, bool calledFromCallback) {

    if (calledFromCallback && verbose)  std::cout << "\t"; // Extra indentation when printing
    if (!calledFromCallback || verbose) std::cout << "\tSolving All Sub Problems..." << std::endl;
    ...
    if (calledFromCallback && verbose)  std::cout << "\t"; // Extra indentation when printing
    if (!calledFromCallback || verbose) std::cout << "\tSolved All Sub Problems" << std::endl;

    // Generate optimality cuts
    for (int s=0; s<NR_SCENARIOS; s++) {
        // Compute the right-hand side of the optimality cut: e = p[s] * pi[s] @ h[s]
...
void BRP_LShapedMethod::intSolCallback(XpressProblem& p) {
    std::cout << "\tEntering intSolCallback..." << std::endl;

    // // Update the current most-recently found x main solution
    // this->lastXValues_i = p.getSolution(x_i);

    // // Initialize objects to store the optimality cuts in
    // std::vector<double> optCutRhs(NR_SCENARIOS, 0.0);
    // std::vector<std::vector<double>> optCutLhsCoeffs(NR_SCENARIOS, std::vector<double>(NR_1ST_STAGE_VARIABLES, 0.0));

    // // Generate the optimality cuts for the found integer solution
    // generateMultiOptimalityCuts(optCutLhsCoeffs, optCutRhs, true);

    // // Add the optimality cuts to the constraint pool. Note: it is not allowed to
    // // call problem.addConstraint() in a node-callback, as this resets the solver state
    // addConstraintsToPool(optCutLhsCoeffs, optCutRhs);

    std::cout << "\tFinished intSolCallback" << std::endl;
}

/**
 * This function adds a set of constraints and their corresponding right-hand side values to the constraint
 * pool of the BRP_LShapedMethod class.
 * 
 * @param newConstraints A matrix representing the coefficients for new constraints to be added to the pool.
 *                       Rows generally correspond to scenarios, and columns to first-stage variables.
 * @param newRHS A vector representing the right-hand side values for the new constraints.
 */
void BRP_LShapedMethod::addConstraintsToPool(const std::vector<std::vector<double>>& newConstraints,
                                             const std::vector<double>& newRHS) {
    if (newConstraints.size() != newRHS.size())
        throw std::invalid_argument("The number of constraints and the number of RHS values must match.");
    
    for (int cnstr = 0; cnstr < newConstraints.size(); cnstr++) {
        constraintPoolLhsMatrix.push_back(newConstraints[cnstr]);
        constraintPoolRhsValues.push_back(newRHS[cnstr]);
    }
}

/**
 * Clears the constraint pool of the BRP_LShapedMethod class. This function is called after the optimality
 * cuts in the pool have been added to the main problem.
 */
void BRP_LShapedMethod::clearConstraintPool() {
    constraintPoolLhsMatrix.clear();
    constraintPoolRhsValues.clear();
}

/**
 * Adds the constraints from the constraint pool to the main problem, then empties the constraint pool.
 */
void BRP_LShapedMethod::addConstraintPoolToMainProb() {
    if (verbose) std::cout << "\tAdding optimality cuts from pool to the main problem" << std::endl;

    // Loop over the constraint pool
    for (int cut=0; cut<constraintPoolRhsValues.size(); cut++) {
        // e.g. constraint 79 in the pool corresponds to scenario 79%50=29,
        // so variable theta[29] is used in the constraint
        int scenario = cut % NR_SCENARIOS;
        // Add the optimality cut to the main problem
        addOptimalityCutToMainProb(scenario, constraintPoolLhsMatrix[cut], constraintPoolRhsValues[cut]);
    }
    clearConstraintPool();
}
 ...
 *  1. \sum_{j} (y[i][j] - y[j][i]) - u[i] + o[i]  == - d[s][i]                 for all stations i
 *  2.                                u[i]         >=   d[s][i]        - x[i]   for all stations i
 *  3.                                       o[i]  >= - d[s][i] - b[i] + x[i]   for all stations i
...
void BRP_SubProblem::makeInitialSubProbFormulation() {
    ...
    // subProbPtr->callbacks.addMessageCallback(XpressProblem::console);
    ...
    for (int i=0; i<NR_STATIONS; i++) {
        mainProbSolver->h_s_j[s][0*NR_STATIONS + i] = - mainProbSolver->d_s_i[s][i];
        mainProbSolver->h_s_j[s][1*NR_STATIONS + i] = mainProbSolver->d_s_i[s][i];
        mainProbSolver->h_s_j[s][2*NR_STATIONS + i] = - mainProbSolver->d_s_i[s][i] - mainProbSolver->b_i[i];

        // T[s][              : 1*NR_STATIONS][:] = square zero-matrix
        // T[s][1*NR_STATIONS : 2*NR_STATIONS][:] = square identity-matrix
        // T[s][2*NR_STATIONS :              ][:] = negative square identity-matrix
    ...
    for (int i=0; i<NR_STATIONS; i++) {
        lhsExpressions[0*NR_STATIONS + i] = end_of_day_net_recourse_flows[i] - u[i] + o[i];
        lhsExpressions[1*NR_STATIONS + i] = u[i];
        lhsExpressions[2*NR_STATIONS + i] = o[i];
    }
    ...
    for (int i=0; i<NR_STATIONS; i++) {
        ...
        objective.addTerm(mainProbSolver->q1_i[i], u[i]);
        objective.addTerm(mainProbSolver->q2_i[i], o[i]);
    }
...
void BRP_SubProblem::solveSubProblem() {
    ...
    // Enable the keep basis feature to warm-start the LP solve and speed up the optimization
    subProbPtr->controls.setKeepBasis(true);
...
int main() {
    try {
        ...
        XpressProblem mainProb;
        // mainProb.callbacks.addMessageCallback(XpressProblem::console);
